...
class WolffUpdater():
    ...
    '''
    def update_site(self, state, i1, j1, i2, j2):
        size = len(state)
        
        prob = self.calc_prob(state[i1,j1], state[i2,j2]) #probability of accepted
        p_rand = np.random.uniform(0, 1) #random number
        
        #decide if link is accepted
        if p_rand < prob:
            state[i2,j2]*= (-1) #flip state
            
            #check adjacent states
            state = self.update_site(state,i2,j2,i2,(j2+1) % size)
            state = self.update_site(state,i2,j2,i2,(j2-1) % size)
            state = self.update_site(state,i2,j2,(i2+1) % size,j2)
            state = self.update_site(state,i2,j2,(i2-1) % size,j2)
            
        #when finished with this link
        return state
    ...
    def update(self, state):
        ...
        #randomly select a site
        i,j = np.random.randint(0, size, size=2)
        state[i,j] *= (-1) #flip site
        
        #check adjacent states
        state = self.update_site(state,i,j,i,(j+1) % size)
        state = self.update_site(state,i,j,i,(j-1) % size)
        state = self.update_site(state,i,j,(i+1) % size,j)
        state = self.update_site(state,i,j,(i-1) % size,j)
